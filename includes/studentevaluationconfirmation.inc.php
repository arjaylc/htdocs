...
<h1 id="pt-contentheader">Student Evaluation Form Submissions</h1>

<?php
	$DBMaster = new DatabaseMaster();
	$username = $DBMaster->escapeString($_SESSION['username']);

	$query = "SELECT CONCAT(firstname, ' ', lastname) AS submittedBy, username, COALESCE(submitter, 'none') AS status FROM group_member INNER JOIN user USING(username) LEFT JOIN eval_essay ON submitter = username WHERE group_id={$_SESSION['group_id']} AND type='student'";
